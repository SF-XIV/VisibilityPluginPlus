...
public class Configuration: Window
{
	...
	public override void Draw()
	{
		...
		if (ImGui.BeginTable("###cols", 7, ImGuiTableFlags.BordersOuterH))
		{
			...
			// Hrothgar Settings
			ImGui.TableNextColumn();
			ImGui.Text(VisibilityPlugin.Instance.PluginLocalization.OptionHrothgars);
			ImGui.TableNextColumn();
			// All
			ImGuiElements.CenteredCheckbox(
				configuration.CurrentEditedConfig.HideHrothgar,
				nameof(configuration.CurrentEditedConfig.HideHrothgar));
			ImGui.TableNextColumn();
			// Combat
			ImGuiElements.CenteredCheckbox(
				configuration.CurrentEditedConfig.HideHrothgarInCombat,
				nameof(configuration.CurrentEditedConfig.HideHrothgarInCombat));
			ImGui.TableNextColumn();
			// Party
			ImGuiElements.CenteredCheckbox(
				configuration.CurrentEditedConfig.ShowPartyHrothgar,
				nameof(configuration.CurrentEditedConfig.ShowPartyHrothgar));
			ImGui.TableNextColumn();
			// Friends
			ImGuiElements.CenteredCheckbox(
				configuration.CurrentEditedConfig.ShowFriendHrothgar,
				nameof(configuration.CurrentEditedConfig.ShowFriendHrothgar));
			ImGui.TableNextColumn();
			// FC
			ImGuiElements.CenteredCheckbox(
				configuration.CurrentEditedConfig.ShowCompanyHrothgar,
				nameof(configuration.CurrentEditedConfig.ShowCompanyHrothgar));
			ImGui.TableNextColumn();
			// Dead
			ImGuiElements.CenteredCheckbox(
				configuration.CurrentEditedConfig.ShowDeadHrothgar,
				nameof(configuration.CurrentEditedConfig.ShowDeadHrothgar));
			ImGui.TableNextRow();
		...
		// Bottom window buttons (Refresh Instance/Voidlist/Whitelist)
