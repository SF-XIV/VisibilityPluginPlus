...
public class FrameworkHandler: IDisposable
{
	...
	private readonly HrothgarHandler hrothgarHandler;
	...
	public FrameworkHandler()
	{
		...
		this.hrothgarHandler = new HrothgarHandler(this.containerManager, this.voidListManager, this.visibilityManager);
	...
	public unsafe void Update()
	{
		...
		for (int i = 1; i != 200; ++i)
		{
			...
			{
				// Hrothgar process
				case ObjectKind.Player when characterPtr->DrawData.CustomizeData.Race == 7:
					this.hrothgarHandler.ProcessHrothgar(characterPtr, localPlayer, isBound);
					break;
				// Player process
	...
	/// <summary>
	/// Check if a character is a Hrothgar
	/// </summary>
	public static unsafe bool CheckTargetIsHrothgar(Character* ptr)
	{
		if(ptr == null || !ptr->IsCharacter())
		{
			return false;
		}

		return ptr->DrawData.CustomizeData.Race == 7;
	}
	...
	/// <summary>
	/// Show all hrothgars in a specific container
	/// </summary>
	public void ShowHrothgars(ContainerType type) => this.Show(UnitType.Hrothgars, type);
	...
	/// <summary>
	/// Show a specific hrothgar by ID
	/// </summary>
	public void ShowHrothgar(uint id)
	{
